...
final class Attachment_Taxonomies_Core {
	...
	public function render_taxonomy_filters( $post_type ) {
		...
		foreach ( $this->get_taxonomies( 'objects' ) as $taxonomy_slug => $taxonomy ) {
			...
			?>
			<label for="attachment-<?php echo sanitize_html_class( $taxonomy->query_var ); ?>-filter" class="screen-reader-text"><?php echo esc_html( $this->get_filter_by_label( $taxonomy ) ); ?></label>
